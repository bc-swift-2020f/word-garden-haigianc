...
class ViewController: UIViewController {
    ...
    func formatRevealedWord(){
        //format and show revealedWord in wordsBeingRevealedLabel to include new guess
        var revealedWord = ""
        ...
        if wordToGuess.contains(currentLetterGuessed) == false{
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.25) {
                ...
                { (_) in
                    ...
                    //if we are not the last flower, show the next flower
...
//        }
    ...
    @IBAction func playAgainButtonPressed(_ sender: UIButton) {
        //if all the words have been guessed and you select playAgain, then restart all games as if the app has been restarted
        if currentWordIndex == wordsToGuess.count{
            currentWordIndex = 0
            wordsGuessedCount = 0
            wordsMissedCount = 0
        ...
        guessedLetterButton.isEnabled = false //don't turn true until theres a character in the text field
        wordToGuess = wordsToGuess[currentWordIndex]
        wrongGuessesRemaining = maxNumberOfWrongGuesses
        //creat word with underscores, one for each space
        wordsBeingRevealedLabel.text = "_" + String(repeating: " _", count: wordToGuess.count-1)
        guessCount = 0
